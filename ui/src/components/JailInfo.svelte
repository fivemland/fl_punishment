...
{#if jail && typeof jail === 'object'}
  <main id="infopanel" class="bg-gray-700 p-2 rounded-xl select-none">
    ...
    <InfoBar title={$_('time_left')} value={`${(jail.all || 0) - (jail.count || 0)} minutes`} />
    <InfoBar title={$_('all_time')} value={`${jail.all} minutes`} />
    <InfoBar title={$_('reason')} value={jail.reason} />
    <InfoBar title={$_('admin')} value={jail.admin ? jail.admin.name : 'Unknown'} />
